(() => {
 ...
 async function removeVisit(id,button){
 ...
 async function load(){if(loading)return;loading=true;$('status').textContent='Consultando métricas…';try{
  ...
  d.recent.forEach(r=>{const el=node('details','');el.append(node('summary',new Date(r.created_at).toLocaleString('pt-BR')+' · '+r.device+' · '+r.active_seconds+' s · rolagem '+r.scroll+'%'));const del=node('button','Excluir visita');del.type='button';del.className='delete-visit';del.onclick=()=>removeVisit(r.id,del);el.append(node('p','Visita: '+r.id),node('p','Origem: '+r.source+' · Criativo: '+r.creative),node('p','Seções: '+(r.sections.map(label).join(' • ')||'Nenhuma registrada')),node('p','Cliques: '+(r.clicks.map(label).join(' • ')||'Nenhum registrado')),del);$('recent').append(el);});
